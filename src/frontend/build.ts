...
export async function pipeline(env: EnvSchemaType, branchName: string, commitId: string, force: boolean = false) {
  ...
  pipe.use(async (ctx, next) => {
    ...
    const childHandler = exec('pnpm i', { cwd: env.STREAMS2_FRONTEND });
  ...
  pipe.use(async (ctx, next) => {
    ...
    console.log('start to run pnpm install');
    const handler2 = exec('pnpm run lint', { cwd: env.STREAMS2_FRONTEND });
  ...
  pipe.use(async (ctx, next) => {
    ...
    const handler = exec('pnpm run build', { cwd: env.STREAMS2_FRONTEND });
...
export class Pipeline {
  ...
  async init(commitId: string, branchName: string, force: boolean) {
    ...
    let flags: 'w+' = 'w+';
    try {
      stat = statSync(`${this.env.LOG_LOCATION}/${commitId}`);
    } catch { }
    if (stat && !force) {
    ...
    const wStream = createWriteStream(`${this.env.LOG_LOCATION}/${commitId}.log`, { flags, autoClose: false });

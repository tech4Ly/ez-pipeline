...
const apiSetActiveCommit = nl.post(
    "/pipeline/streams2/nl/activeBranch/:commitId",
    async (c) => {
        const { commitId } = c.req.param();
        const myEnv = env(c);
        const state = await PipelineState.init(myEnv);
        const { availableBranches, activePID } =
            state.readByPipelineName(NOTIFICATION_LETTER);
...
const apiTriggerPipeline = nl.post(
    "/pipeline/streams2/nl/:branchName/:commitId",
    async (c) => {
        ...
        try {
            await triggerPull(
                myEnv.EZ_PIPELINE_STREAMS2_NOTIFICATION_LETTER,
                commitId,
            );
            // 开始触发, 触发后不需要等待它结束
...
const apiGetPipelineDetail = nl.get(
    "/pipeline/streams2/nl/:branchName/:commitId",
    async (c) => {
        const { commitId } = c.req.param();
        const log = await getLogText(

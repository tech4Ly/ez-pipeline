...
const apiSetActiveCommit = str.post(
    "/pipeline/streams2/str/activeBranch/:commitId",
    async (c) => {
...
const apiTriggerPipeline = str.post(
    "/pipeline/streams2/str/:branchName/:commitId",
    async (c) => {
...
const apiGetPipelineDetail = str.get(
    "/pipeline/streams2/str/:branchName/:commitId",
    async (c) => {
        const { commitId } = c.req.param();
        const log = await getLogText(

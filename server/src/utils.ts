...
export function promiseFromChildProcess(child: ChildProcess) {
    return new Promise<[number | null, NodeJS.Signals | null]>(
        (resolve, reject) => {
...
export class PipelineState {
    ...
    async readFromDisk(): Promise<StateType> {
        const jsonFile = await fs.readFile(
            `${this.env.EZ_PIPELINE_STATE_LOCATION}/state.json`,
            {
                encoding: "utf8",
            },
        );
    ...
    flush() {
        if (this._stateQueque.length > 0) {
            ...
            writeFileSync(
                `${this.env.EZ_PIPELINE_STATE_LOCATION}/state.json`,
                JSON.stringify(this._stateQueque.pop()),
            );
        }
    }
    ...
    updateStateByKey<K extends keyof StateElementType>(
        name: string,
        key: K,
        value: StateElementType[K],
    ) {
    ...
    updateBuildStatus(name: string, commitId: string, status: BuildStatus) {
        const state = this.state.find((s) => s.pipelineName === name);
        if (state) {
            const targetStatusIdx = state.buildStatus.findIndex(
                (b) => b.commitId === commitId,
            );

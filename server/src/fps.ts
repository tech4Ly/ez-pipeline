...
const apiSetActiveCommit = fps.post(
    "/pipeline/streams2/fps/activeBranch/:commitId",
    async (c) => {
...
const apiTriggerPipeline = fps.post(
    "/pipeline/streams2/fps/:branchName/:commitId",
    async (c) => {
...
const apiGetPipelineDetail = fps.get(
    "/pipeline/streams2/fps/:branchName/:commitId",
    async (c) => {
        const { commitId } = c.req.param();
        const log = await getLogText(
